...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		public ChessPiece replacePromotedPiece (String type) {
			if(promoted == null ) {
				throw new IllegalStateException("There is o piece to be promoted");
			}
	...
	private void validateSourcePosition(Position position) {
		...
		private void validateTargetPosition(Position source, Position target) {
			if(!board.piece(source).possibleMove(target)) {
				throw new ChessException("The chosen piese can't move to target position");
			}
		}
			...
			for (int i= 0; i<board.getRows();i++) {
				for (int j=0; j<board.getColumns();j++) {
					if (mat[i][j]){
						...
						Piece capturedPiee = makeMove(source,target);
						boolean testCheck = testCheck(color);
						undoMove(source, target, capturedPiee);
						if (!testCheck) {
							return false;
						}
					}
